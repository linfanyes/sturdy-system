...
### 2026-08-10 · 边界用例补充 + 教师自助改密（第二轮）

**边界测试（41 例新增，累计 558 例全绿）**
- 后端 `server/qa/edge.ts`：四角色边界 38 例（异常输入/越权/隔离/容错/安全载荷）。
- Web UX 边界 3 例：作业无截止日期、考试无分数、旧版历史账号兼容。

**新发现并修复**
- 【高】成绩录入无分数范围校验（负分可落库）→ importGrades 过滤 0-1000 之外分数（缺考 null 允许）。
- 【高】成绩录入不校验学生班级归属（可跨班写入）→ 按本班学生集合过滤。
- 【产品缺口补齐】教师自助改密：`POST /auth/change-password`（校验原密码、8-20 位、审计日志）+ Web 设置页新增「账号安全」标签页。

### 2026-08-10 · 全系统测试体系重建 + 7 项缺陷修复

**测试体系（qa/）**
- 新增 QA 测试中心 `qa/`：测试案例目录（四级用户全功能/性能/交互，517 例）、测试数据设计、测试报告。
- 后端自动化 `server/qa/`：内存库（better-sqlite3）真实 NestJS 服务 + 确定性种子数据集（10 校 × 20 师 × 10 班 × 50 生 × 10 考试，含进步/退步/偏科/缺考等多分布场景）+ 65 功能用例 + 6 性能用例（登录吞吐 p95 569ms、看板聚合 270 RPS、混合并发 0 错误）。
- Web 自动化：交互体验用例（登录页 9 例、家长看板 11 例）+ 183 页面全量渲染冒烟 + 路由权限，共 242 例全绿。
- 小程序自动化：家长登录安全属性、学号规则与默认口令跨端一致性，共 204 例全绿。

**缺陷修复（详见 qa/TEST_REPORT.md）**
1. 【严重】shared 包 ESM 编译致服务端运行时无法解析（部署后启动即崩溃风险）→ 改 CommonJS 编译 + 补全 exports + vendor 同步脚本（`server/scripts/sync-shared.cjs`）。
2. 【严重】校管「成绩查询与汇总」恒 500（ClassItem 误用 classId 条件）→ 按主键查询。
3. 【严重】纯家长统一登录 JWT 缺 parentId 致 /me 返回 null → 登录时回填并签发 parentId。
4. 【高】无 classId 列实体（checkins 等）按班级查询恒 500 → CrudService 按实体元数据条件过滤。
5. 【高】小程序家长登录仅允许纯数字学号（与学号管理/统一登录不一致）→ 统一 isStudentNo 口径。
6. 【中】小程序默认口令展示「学号后6位」与后端 123456 不符 → 展示修正。
7. 【低】shared exports 缺 9 个子路径 → 补全。

**工程**
- 登录限流支持 `LOGIN_RATE_LIMIT_MAX` 环境配置（生产默认 10 次/分钟不变），QA 压测可放宽。
- CI 保持：server/web 类型检查、web jest、mini jest、双端构建与主包体积门禁。
