    ...
    rollupOptions: {
      output: {
        ...
        manualChunks(id) {
          ...
          // P0-9修复：使用绝对路径前缀匹配 monorepo 根下的 shared 目录，避免误匹配 node_modules 内其他 shared 目录
          const sharedDir = path.resolve(__dirname, '../shared').replace(/\\/g, '/')
          if (fid.includes('@gardener/shared') || fid.startsWith(sharedDir + '/')) {
