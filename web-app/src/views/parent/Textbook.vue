...
/**
 * 家长端教材知识点浏览：按学科/年级筛选 + 关键词检索 + 树形展开浏览
 */
import { ref, computed, onMounted } from 'vue'
import { BookOpen, ChevronRight, ChevronDown, Search, Loader2, X } from 'lucide-vue-next'
...
async function doSearch() {
  ...
  try {
    searchResults.value = await searchKnowledgePoints(searchKeyword.value.trim())
...
<template>
  <div class="space-y-4 max-w-5xl mx-auto">
    <h1 class="text-2xl font-bold text-cocoa-900 flex items-center gap-2">
      <BookOpen class="w-6 h-6 text-butter-500" /> 教材知识点
