 ...
 * - 已登录但角色不匹配 → 跳转 403
 *
 * 教师子路由按业务模块拆分子数组，便于维护：
 *   - teacherPersonal: 个人空间（资料/设置/待办/笔记）
 *   - teacherClass: 班级与学生管理
 *   - teacherExam: 学情与考试（考试/成绩/分析）
 *   - teacherEval: 学生评价（奖励/成长/行为/阅读）
 *   - teacherHome: 家校沟通
 *   - teacherAI: AI 与备课
 *   - teacherOffice: 教师办公
 *   - teacherTools: 工具箱（课堂互动/学科工具）
 *   - teacherGames: 游戏合集
 *   - teacherOther: 其他功能模块
 */

// —— 教师子路由：个人空间 ——
const teacherPersonal: RouteRecordRaw[] = [
...
// —— 教师子路由：班级与学生 ——
const teacherClass: RouteRecordRaw[] = [
...
// —— 教师子路由：学情与考试 ——
const teacherExam: RouteRecordRaw[] = [
...
// —— 教师子路由：学生评价 ——
const teacherEval: RouteRecordRaw[] = [
...
// —— 教师子路由：家校沟通 ——
const teacherHome: RouteRecordRaw[] = [
...
// —— 教师子路由：AI 与备课 ——
const teacherAI: RouteRecordRaw[] = [
...
// —— 教师子路由：教师办公 ——
const teacherOffice: RouteRecordRaw[] = [
...
// —— 教师子路由：工具箱（课堂互动 + 学科工具） ——
const teacherTools: RouteRecordRaw[] = [
...
// —— 教师子路由：游戏合集 ——
const teacherGames: RouteRecordRaw[] = [
...
// —— 教师子路由：其他功能模块 ——
const teacherOther: RouteRecordRaw[] = [
  ...
  { path: 'schedule', name: 'teacher-schedule', component: () => import('@/views/teacher/Schedule.vue'), meta: { title: '课表与调课' } },
  { path: 'report', name: 'teacher-report', component: () => import('@/views/teacher/Report.vue'), meta: { title: '班级周/月报' } },
  { path: 'assignment', name: 'teacher-assignment', component: () => import('@/views/teacher/Assignment.vue'), meta: { title: '分层作业布置' } },
