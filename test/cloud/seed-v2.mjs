/**
 * 云端数据写入脚本 v2：5 学校 × 6 年级 × 3 班级 × 30 学生 × 10 考试
 * 使用云托管 API 直接写入
 */
...
async function req(method, path, { token, body, retries = 3 } = {}) {
  ...
  for (const school of SCHOOLS) {
    ...
    for (let attempt = 1; attempt <= 5; attempt++) {
      r = await req('POST', '/admin/school-admins', {
        token: superToken,
        body: { username: adminUser, password: PASSWORD, name: `${school.name}校管`, schoolId: school.id },
      })
      ...
      if (attempt < 5) {
        console.log(`  ⚠️ 校管创建重试 (${attempt}/5): ${JSON.stringify(r.data).slice(0, 80)}`)
        await sleep(2000 * attempt)
      } else {
        console.log(`  ❌ 校管创建失败: ${JSON.stringify(r.data).slice(0, 80)}`)
      }
    }
    ...
    if (r.data?.id) {
      ...
      if (r.data?.token) {
        sadToken = r.data.token
      }
    }

    if (!sadToken) {
      console.log(`  ❌ 校管登录失败，跳过`)
      continue
    }
    ...
    for (const g of GRADES) {
      for (let c = 1; c <= 3; c++) {
        teachers.push({ name: `${g}${c}班班主任`, username: `ht_${school.prefix}_${g}_${c}`, password: PASSWORD, subject: '语文', grade: g })
      }
    }
    for (const g of GRADES) {
