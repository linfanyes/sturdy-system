 ...
 */
export function fmt1(n: number | string | null | undefined): string {
 ...
 */
export function pctNum(n: number | string | null | undefined): string {
  if (n == null || n === '' || isNaN(Number(n))) return '-'
  return Number(n).toFixed(1) + '%'
}
...
export function scoreColor(score: number | null | undefined, fullScore: number = 100): string {
  ...
  if (p >= 0.6) return '#f5d342'
  if (p >= 0.5) return '#f5b342'
  return '#f56c6c'
}