 ...
 */
export function buildWordHtml(options: BuildWordHtmlOptions): string {
 ...
 * 仅拼接纯字符串，不含平台 I/O。
 */
export function buildHeaderBlock(fields: Record<string, string | number | null | undefined>): string {
 ...
 */
export function composeDocContent(
 ...
 *   ws['!cols'] = colWidths
 */
export function computeXlsxLayout(
  header: string[],
  rows: string[][],
  opts?: { maxColWidth?: number; padding?: number },
): { colWidths: { wch: number }[]; totalCols: number; totalRows: number } {
  const maxColWidth = opts?.maxColWidth ?? 40
  const padding = opts?.padding ?? 2
  const colWidths = header.map((_, ci) => {
    let max = (header[ci] || '').length * 2
    rows.forEach((r) => {
      if (r[ci] != null) max = Math.max(max, String(r[ci]).length * 2)
    })
