 ...
 */
/** 游戏得分提交 / 查询 endpoints */
export declare const GAME_SCORES_PATHS: {
    /** POST /game-scores —— 上报一次对局得分（幂等 upsert） */
    readonly submit: "/game-scores";
    /** GET /game-scores —— 当前教师所有游戏得分（榜单） */
    readonly list: "/game-scores";
    /** GET /game-scores/:gameKey —— 单游戏最高分 */
    readonly byKey: (gameKey: string) => string;
};
...
export declare const CHAT_SESSIONS_PATHS: {
    /** POST /chat-sessions —— 新建会话 */
    readonly create: "/chat-sessions";
    /** GET /chat-sessions —— 会话列表 */
    readonly list: "/chat-sessions";
    /** GET /chat-sessions/:id —— 会话详情（含全部消息） */
    readonly byId: (id: string) => string;
    /** PATCH /chat-sessions/:id/messages —— 追加一条消息 */
    readonly appendMessage: (id: string) => string;
    /** PATCH /chat-sessions/:id/pin —— 置顶/取消置顶 */
    readonly togglePin: (id: string) => string;
