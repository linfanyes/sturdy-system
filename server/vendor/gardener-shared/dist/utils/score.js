...
exports.computeExamStats = computeExamStats;
exports.normalizeDistribution = normalizeDistribution;
exports.toSubjectNames = toSubjectNames;
/**
 ...
 */
function computeExamStats(scores, fullScore = 100) {
    const empty = {
        avg: '-',
        max: '-',
        min: '-',
    ...
    const all = scores.filter((x) => {
        const v = typeof x === 'object' && x ? x.score : x;
        return v != null;
    });
    const sc = all.map((x) => Number(typeof x === 'object' && x ? x.score : x));
    if (!sc.length)
        return empty;
    sc.sort((a, b) => a - b);
 ...
 */
function toSubjectNames(arr) {
    if (!Array.isArray(arr))
        return [];
    return arr
        .map((x) => (typeof x === 'string' ? x : (x && x.subject) || (x && x.name) || ''))
        .filter(Boolean);
}
