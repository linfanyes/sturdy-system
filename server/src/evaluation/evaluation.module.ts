...
export class LeaderboardController {
  ...
  async getLeaderboard(
    ...
    if (!cls) {
      throw new BadRequestException('班级不存在或无权访问')
    }

    // 并行查询加分和减分记录
    const [scores, rewards] = await Promise.all([
      this.scoreRepo.find({ where: { classId } as any, take: 500 }),
      this.rewardRepo.find({ where: { classId } as any, take: 500 }),
    ])
    ...
    for (const r of scores) {
      ...
      if (!sid) continue
      const existing = map.get(sid) || { studentId: sid, name: r.studentName || studentMap.get(sid) || '未知', total: 0, count: 0 }
      existing.total += Number(r.delta || 0)
      existing.count += 1
      existing.name = existing.name || r.studentName || studentMap.get(sid) || '未知'
      map.set(sid, existing)
    }
    for (const r of rewards) {
      const sid = r.studentId
      if (!sid) continue
      const existing = map.get(sid) || { studentId: sid, name: studentMap.get(sid) || '未知', total: 0, count: 0 }
      const delta = r.type === '减分' ? -Number(r.points || 0) : Number(r.points || 0)
      existing.total += delta
      existing.count += 1
      existing.name = studentMap.get(sid) || '未知'
      map.set(sid, existing)
    }
