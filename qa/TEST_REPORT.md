...
> 执行时间：2026-08-10　|　执行人：QA 自动化　|　被测版本：`master`
> 结论：**两轮测试后三端 558 例全部通过；累计发现并修复 10 个缺陷 + 补齐 1 项产品缺口（教师自助改密）。**
...
另修正 1 处用例设计问题：EDGE-PAR-04 曾以 6 位默认口令作为改密还原值，与"新密码≥8位"策略冲突并污染后续性能用例，已调整为独立学生账号。

说明：EDGE-SA-06 确认 `/school-admin/students` 搜索为前端本地过滤（服务端不处理 keyword），已按真实行为校准断言，非缺陷。
