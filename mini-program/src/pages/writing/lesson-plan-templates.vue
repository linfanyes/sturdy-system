<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="hd">教案模板</view>
    <view class="bar">
      <text class="bar-text">管理教案模板，支持按学科、年级分类</text>
    ...
    <view v-else class="list">
      <view v-for="it in list" :key="it.id" class="item">
        ...
        <view class="meta">
          <text>{{ it.grade }}</text>
          <text v-if="it.createdAt">{{ it.createdAt }}</text>
        </view>
    ...
    <view v-if="showForm" class="mask" @click="showForm = false">
      <view class="sheet safe-bottom" @click.stop>
        ...
        <view class="field">
          ...
          <input v-model="form.name" class="inp" placeholder="如：小学语文标准教案模板" />
        </view>
        <view class="field">
        ...
        <view class="field">
          ...
          <textarea v-model="form.content" class="inp area" placeholder="教案模板内容" />
        </view>
        <view class="btn-row">
...
const form = ref({ name: '', subject: '', grade: '', content: '' })
...
function openAdd() {
  editing.value = null
  form.value = { name: '', subject: '', grade: '', content: '' }
  showForm.value = true
}

function openEdit(it) {
  editing.value = it
  form.value = { name: it.name, subject: it.subject || '', grade: it.grade || '', content: it.content || '' }
  showForm.value = true
}

async function submit() {
  if (!form.value.name.trim()) return uni.showToast({ title: '请填写模板名称', icon: 'none' })
  saving.value = true
  try {
    if (editing.value) {
