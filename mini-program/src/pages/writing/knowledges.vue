<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="hd">知识点库</view>
    <view class="bar">
      <text class="bar-text">管理学科知识点，支持按学科、年级分类</text>
      <text class="act" @click="openAdd">＋ 添加</text>
    </view>

    <view v-if="loading" class="loading">加载中…</view>
    <view v-else-if="!list.length" class="empty">暂无知识点，点击添加</view>
    <view v-else class="list">
      <view v-for="it in list" :key="it.id" class="item">
        ...
        <view v-if="it.createdAt" class="meta2">{{ it.createdAt }}</view>
        <view class="ops">
    ...
    <view v-if="showForm" class="mask" @click="showForm = false">
      <view class="sheet safe-bottom" @click.stop>
        <view class="sh-t">{{ editing ? '编辑' : '新增' }}知识点</view>
        <view class="field">
          <text class="label">标题 *</text>
          <input v-model="form.title" class="inp" placeholder="如：分数的基本性质" />
        </view>
        <view class="field">
        ...
        <view class="field">
          ...
          <input v-model="form.topic" class="inp" placeholder="如：分数的加减法" />
        </view>
        <view class="field">
          <text class="label">内容</text>
          <textarea v-model="form.content" class="inp area" placeholder="知识点详细内容" />
        </view>
        <view class="btn-row">
...
const form = ref({ title: '', subject: '', grade: '', topic: '', content: '' })
...
function openAdd() {
  editing.value = null
  form.value = { title: '', subject: '', grade: '', topic: '', content: '' }
  showForm.value = true
}

function openEdit(it) {
  editing.value = it
  form.value = { title: it.title, subject: it.subject || '', grade: it.grade || '', topic: it.topic || '', content: it.content || '' }
  showForm.value = true
}
...
.meta2 { font-size: 22rpx; color: var(--c-sub); margin-top: 4rpx; }
