<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="hd">加减分</view>
    <view class="bar">
      <text class="bar-text">管理学生加减分记录</text>
      <text class="act" @click="openAdd">＋ 添加</text>
    </view>

    <view v-if="loading" class="loading">加载中…</view>
    <view v-else-if="!list.length" class="empty">暂无加减分记录，点击添加</view>
    <view v-else class="list">
      <view v-for="it in list" :key="it.id" class="item">
        <view class="it-top">
          <text class="name">{{ it.studentName }}</text>
          <text :class="['score', it.type === '减分' ? 'red' : 'green']">
            {{ it.type === '减分' ? '-' : '+' }}{{ it.score }}
          </text>
        </view>
        <view class="meta">
          ...
          <text>{{ it.date }}</text>
          <text :class="it.type === '减分' ? 'red' : ''">{{ it.type }}</text>
        </view>
    ...
    <view v-if="showForm" class="mask" @click="showForm = false">
      <view class="sheet" @click.stop>
        ...
        <view class="field">
          ...
          <picker :range="types" :value="typeIdx" @change="onTypeChange">
            <view class="picker">{{ form.type || '请选择' }}</view>
          </picker>
        </view>
        <view class="field">
          <text class="label">原因 *</text>
          <input v-model="form.reason" class="inp" placeholder="加减分原因" />
        </view>
        <view class="field">
          <text class="label">分值 *</text>
          <input v-model.number="form.score" class="inp" type="number" placeholder="输入分值" />
        </view>
        <view class="field">
...
const types = ['加分', '减分']
const typeIdx = ref(0)
const form = ref({ classId: '', className: '', studentName: '', date: '', type: '加分', reason: '', score: 1, note: '' })
...
function openAdd() {
  editing.value = null
  form.value = { classId: '', className: '', studentName: '', date: '', type: '加分', reason: '', score: 1, note: '' }
  classIdx.value = 0; typeIdx.value = 0
...
function openEdit(it) {
  ...
  typeIdx.value = types.indexOf(it.type); if (typeIdx.value < 0) typeIdx.value = 0
  showForm.value = true
}

async function submit() {
  if (!form.value.studentName.trim()) return uni.showToast({ title: '请填写学生姓名', icon: 'none' })
  if (!form.value.reason.trim()) return uni.showToast({ title: '请填写原因', icon: 'none' })
  saving.value = true
  try {
    if (editing.value) {
