<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    ...
    <view class="bar">
      <text class="bar-text">查询和管理试卷信息</text>
      <text class="act" @click="openAdd">＋ 添加</text>
    </view>

    <view v-if="loading" class="loading">加载中…</view>
    <view v-else-if="!list.length" class="empty">暂无查询记录，点击添加</view>
    <view v-else class="list">
      <view v-for="it in list" :key="it.id" class="item">
        ...
        <view v-if="it.query" class="desc">{{ it.query }}</view>
        <view v-if="it.result" class="result">{{ it.result }}</view>
        <view v-if="it.createdAt" class="meta">{{ it.createdAt }}</view>
        <view class="ops">
    ...
    <view v-if="showForm" class="mask" @click="showForm = false">
      <view class="sheet" @click.stop>
        <view class="sh-t">{{ editing ? '编辑' : '新增' }}查询</view>
        <view class="field">
          <text class="label">标题 *</text>
          <input v-model="form.title" class="inp" placeholder="如：三年级数学期末试卷" />
        </view>
        <view class="field">
        ...
        <view class="field">
          <text class="label">查询内容</text>
          <textarea v-model="form.query" class="inp area" placeholder="查询内容或关键词" />
        </view>
        <view class="field">
          <text class="label">结果</text>
          <textarea v-model="form.result" class="inp area" placeholder="查询结果" />
        </view>
        <view class="btn-row">
...
const form = ref({ title: '', subject: '', query: '', result: '' })
...
function openAdd() {
  editing.value = null
  form.value = { title: '', subject: '', query: '', result: '' }
  showForm.value = true
}

function openEdit(it) {
  editing.value = it
  form.value = { title: it.title, subject: it.subject || '', query: it.query || '', result: it.result || '' }
  showForm.value = true
}
...
.result { font-size: 24rpx; color: var(--c-title); margin-top: 8rpx; background: var(--c-card2); padding: 12rpx; border-radius: 8rpx; overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; }
