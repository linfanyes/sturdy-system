<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="topbar"><text class="mtitle">🔊 拼音标注</text></view>
    <view class="card">
      <view class="sec-title">汉字拼音标注</view>
      <textarea v-model="text" class="ctrl area" placeholder="输入需要标注拼音的汉字或文章" />
      <button class="gen" :disabled="loading || !text" @click="generate">{{ loading ? '标注中…' : '标注拼音' }}</button>
