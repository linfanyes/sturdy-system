<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="topbar"><text class="mtitle">📖 阅读理解</text></view>
    <view class="card">
      <view class="sec-title">阅读理解材料生成</view>
      <input v-model="form.topic" class="ctrl" placeholder="主题（如：环保、友谊、春天）" />
      <picker :range="gradeOpts" @change="form.grade = gradeOpts[$event.detail.value]">
        <view class="ctrl pick">年级：{{ form.grade || '选择年级' }}</view>
      </picker>
      <button class="gen" :disabled="loading || !form.topic" @click="generate">{{ loading ? '生成中…' : '生成阅读材料' }}</button>
...
const form = ref({ topic: '', grade: '' })
const result = ref('')
const loading = ref(false)
async function generate() {
  loading.value = true
  try {
    const prompt = `请为${form.value.grade||'小学'}学生生成一篇关于「${form.value.topic}」的阅读理解短文（200-400字），附 3 道理解题（含答案）。`
