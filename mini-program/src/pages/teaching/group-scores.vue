<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="hd">小组评分</view>
    <view class="bar">
      <text class="bar-text">管理各班级小组评分</text>
      <text class="act" @click="openAdd">＋ 添加</text>
    </view>

    <view v-if="loading" class="loading">加载中…</view>
    <view v-else-if="!list.length" class="empty">暂无小组评分记录，点击添加</view>
    <view v-else class="list">
      <view v-for="it in list" :key="it.id" class="item">
        <view class="it-top">
          <text class="name">{{ it.groupName }}</text>
        ...
        <view v-if="it.reason" class="desc">{{ it.reason }}</view>
        <view class="ops">
    ...
    <view v-if="showForm" class="mask" @click="showForm = false">
      <view class="sheet safe-bottom" @click.stop>
        <view class="sh-t">{{ editing ? '编辑' : '新增' }}小组评分</view>
        <view class="field">
        ...
        <view class="field">
          <text class="label">小组名 *</text>
          <input v-model="form.groupName" class="inp" placeholder="如：第一组" />
        </view>
        <view class="field">
        ...
        <view class="field">
          ...
          <input v-model.number="form.score" class="inp" type="number" placeholder="输入分数" />
        </view>
        <view class="field">
          <text class="label">原因</text>
          <input v-model="form.reason" class="inp" placeholder="评分原因" />
        </view>
        <view class="field">
...
const form = ref({ classId: '', className: '', groupName: '', date: '', score: 1, reason: '', note: '' })
...
function openAdd() {
  editing.value = null
  form.value = { classId: '', className: '', groupName: '', date: '', score: 1, reason: '', note: '' }
  classIdx.value = 0
  if (classes.value[0]) {
    form.value.classId = classes.value[0].id
    form.value.className = classes.value[0].name
  }
  showForm.value = true
}
...
function openEdit(it) {
  editing.value = it
  form.value = { ...it }
  classIdx.value = classes.value.findIndex(c => c.id === it.classId)
  if (classIdx.value < 0) classIdx.value = 0
  showForm.value = true
}

async function submit() {
  if (!form.value.groupName.trim()) return uni.showToast({ title: '请填写小组名', icon: 'none' })
