<template>
  <view class="page" :class="{ dark }">
    <view class="card">
      <view class="sec-title">{{ tool ? tool.icon + ' ' + tool.title : '智能工具' }}</view>
      <view class="hint">{{ tool ? tool.hint : '填写下方信息，AI 一键生成，可复制使用。' }}</view>
...
import { getQuickTool } from '../../common/quicktool-schema'
...
onLoad((q) => {
  ...
  tool.value = getQuickTool(type.value)
  if (tool.value) {
