<template>
  <view class="page" :class="{ dark: theme.mode === 'dark' }">
    <view class="hd">📝 备课记录</view>

    <view class="bar">
      <text class="sc" v-if="list.length">共 {{ list.length }} 条备课记录</text>
    </view>

    <Skeleton v-if="loading" :rows="3" />

    <EmptyState v-else-if="!list.length" icon="📝" text="暂无备课记录" hint="使用 AI 备课中心生成教案" />

    <view v-else class="list">
      <view class="item" v-for="it in list" :key="it.id" @click="openDetail(it)">
        <view class="it-top">
          <text class="it-title">{{ it.title || '未命名教案' }}</text>
