...
REM  前提: 本地后端服务跑在 localhost:3000（如 npm run start / node server）
...
REM 1/3 切换后端配置为本地
echo [1/3] 切换 config.js -^> 本地联调（/api -> localhost:3000）
node "%SCRIPT_DIR%scripts\set-web-env.js" local
...
echo   注意: 本模式需要本地后端已在 localhost:3000 运行，否则接口会 404。
echo   日志:   %LOG_FILE%
